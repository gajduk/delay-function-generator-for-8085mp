		...
		for ( Element el  : elements_usable ) {
			...
			if (  el.type == Element.MEMORY ) {
				if ( memory_locations_counter < 3 && 
						el.status.equals(Element.available) ) {
					int memory_location = Integer.parseInt(el.description);
					if ( memory.contains(memory_location-1) || memory.contains(memory_location+1) ) {
						e += memory_encoding[memory_locations_counter++];
					}
					if ( memory_locations_counter < 2 ) {
						e += memory_encoding[memory_locations_counter++];
					}
					memory.add(memory_location);
				}
			}
			else {
			if ( el.type == Element.STACK ) {
				if ( el.status.equals(Element.available) && el.description.equals(Element.STACK_NOT_VALID) ) {
